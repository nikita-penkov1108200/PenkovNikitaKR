...
{
    public partial class DobavlenieZakazov : Form
    {

        public DobavlenieZakazov()
        {
            InitializeComponent();
        ...
        private void comboBoxClientName_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                string[] names = selectedName.Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
                ...
                if (names.Length >= 3)
                {
                    ...
                    {
                        con.Open();
                        string query = "SELECT PhoneNumber, StatusVIP FROM client WHERE Name = @Name AND Surname = @Surname AND MiddleName = @MiddleName";
                        MySqlCommand cmd = new MySqlCommand(query, con);
                        ...
                        using (MySqlDataReader reader = cmd.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                // Если клиент существует, заполняем поля
                                textBoxPhoneNumber.Text = reader["PhoneNumber"].ToString();
                                comboBoxVIPStatus.SelectedValue = reader["StatusVIP"].ToString();
                            }
                            else
                            {
                                // Если клиента нет, очищаем поля
        ...
        // Метод для получения последнего номера заказа
        private int GetLastOrderNumber()
        {
            using (MySqlConnection con = new MySqlConnection(ConnectionString.connectionString()))
            {
                con.Open();
                string query = "SELECT MAX(NumberOrders) FROM orders"; // Получаем максимальный номер заказа
                MySqlCommand cmd = new MySqlCommand(query, con);

                object result = cmd.ExecuteScalar();
                return result != null ? Convert.ToInt32(result) + 1 : 1; // Если нет заказов, начинаем с 1
            }
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            using (MySqlConnection con = new MySqlConnection(ConnectionString.connectionString()))
            {
                ...
                try
                {
                    ...
                    string insertOrderQuery = "INSERT INTO orders (StartDateOrder, EndDateOrder, NameClient, SurnameClient, MiddleNameClient, NumberPhoneClient, StatusVIPClient, FullNameEmployee, Services, CostServices, CostComponentReplaced, TotalCost, NameClientDevice, OrderStatus, client_idClient, services_idservices, orderstatus_idorderstatus, statusvip_idstatusVIP, QuantityComponentReplaced, OrderdDescription, NumberOrders, NameComponentReplaced, employee_idemployee) " +
                                               "VALUES (@StartDate, @EndDate, @NameClient, @SurnameClient, @MiddleNameClient, @NumberPhoneClient, @StatusVIPClient, @FullNameEmployee, @Service, @ServiceCost, @CostComponentReplaced, @TotalCost, @ClientDeviceName, @OrderStatus, @ClientId, @ServiceId, @OrderStatusId, @StatusVIPId, @QuantityComponentReplaced, @Description, @NumberOrders, @NameComponentReplaced, @SotrydnikId)";
        ...
        {
            this.Hide();
            DobavlenieClient.PreviousForm = this;
            DobavlenieClient client1 = new DobavlenieClient();
            client1.Show();
        }
        ...
        {
            this.Hide();
            DobavlenieComponentDevice.PreviousForm1 = this;
            DobavlenieComponentDevice client1 = new DobavlenieComponentDevice();
            client1.Show();
        }
    }
}